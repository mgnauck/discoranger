...
//  FSOUND_Software_SetPan(&FSOUND_Channel[channel], finalpan);
  ...
  if (cptr->notectrl & FMUSIC_FREQ)
  {
    ...
    asm(
      ...
      : "a"(freq), "c"(FSOUND_MixRate), "d"(0));
#else
#warning pls asm this
	ccptr->speedhi = (unsigned long long int)freq / FSOUND_MixRate;
	ccptr->speedlo = ( ( (unsigned long long int)freq % FSOUND_MixRate ) << 32 ) / FSOUND_MixRate;
#endif
#endif		
#ifdef _MSC_VER
    __asm
...
#define READ( dst,size ) \
 { memcpy( dst,buffer + offs,size ); offs+=size; }

int FMUSIC_LoadXM(FMUSIC_MODULE * mod, char * buffer,int size)
{
...
//  FSOUND_File_Read(&filenumpatterns, 2, fp);
  READ( &filenumpatterns,2 );

#if 1				// WARNING! DONT CHANGE THE HEADER AROUND BECAUSE OF THIS HACK.
//  FSOUND_File_Read(&mod->numinsts, 256 + 8, fp);
  READ(&mod->numinsts, 256 + 8);
#else
...
//	FSOUND_File_Read(&dat, 1, fp);
	...
	if (dat & 0x80)
	{
	  if (dat & 1) READ( &nptr->note, 1 );
//	    FSOUND_File_Read(&nptr->note, 1, fp);
	  if (dat & 2) READ( &nptr->number, 1 );
//	    FSOUND_File_Read(&nptr->number, 1, fp);
	  if (dat & 4) READ( &nptr->volume, 1 );
//	    FSOUND_File_Read(&nptr->volume, 1, fp);
	  if (dat & 8) READ( &nptr->effect, 1 );
//	    FSOUND_File_Read(&nptr->effect, 1, fp);
	  if (dat & 16) READ( &nptr->eparam, 1 );
...
//      FSOUND_File_Read(iptr->PANPoints, 48, fp);	// Panning Envelope (24 words)
      READ( &sampHDRsize,4 );
      READ( iptr->keymap, 96 );
      READ( iptr->VOLPoints, 48 );
      READ( iptr->PANPoints, 48 );

#if 1				// WARNING! DONT CHANGE THE HEADER AROUND BECAUSE OF THIS HACK.
//      FSOUND_File_Read(&iptr->VOLnumpoints, 16, fp);
      READ( &iptr->VOLnumpoints, 16 );
#else
